    ...
    $.widget('mageplaza.redirect', {
        ...
        initSaveRedirect: function () {
            ...
            $(".seo-redirect").click(function () {
                ...
                if (typeof targetPath === 'undefined' || targetPath.length === 0) {
                    $('<p class="redirect-required-message">' + $t('This is a required field') +
                        '.</p>').insertAfter(redirectElement + " .target_path");
                } else {
        ...
        sendAjax: function (params, redirectElement) {
            $.ajax({
                ...
                if (response.success) {
                    $('<div class="message message-success"><p>' +
                        response.message + '</p></div>').insertBefore(redirectElement);
