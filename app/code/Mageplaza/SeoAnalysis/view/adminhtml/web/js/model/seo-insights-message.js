    ...
    SeoInsightsMessage = Class.extend({
        ...
        getResult: function () {
            ...
            result.sort(function (a, b) {
                var aState = a.state_class,
                    ...
                    aOrder = aState.search('error') >= 0 ? 1 : aState.search('warning') >= 0 ? 2 : 3,
        ...
        calculateDensity: function (key, description) {
            var self        = this,
                ...
                density     = foundNumber / self.calculateWord(formatDes) * 100;

            density = density.toFixed(1);

            if (density > 1) {
                data.state_class = SeoInsightsMessage.SUCCESS;
                data.message     = $t('Keyword Density is %1%, which is great; the focus keyword was found %2 times.')
                    .replace('%1', density).replace('%2', foundNumber);
            } else {
                data.state_class = SeoInsightsMessage.ERROR;
                data.message     = $t('Keyword Density is %1% (Recommended: 1-3%); the focus keyword was found %2' +
                    ' times.').replace('%1', density).replace('%2', foundNumber);
            }
            this.mp_queue.push(data);
        ...
        evaluateDescriptionContent: function (description) {
            ...
            if (wordNumber > 300) {
                ...
                data.state_class = SeoInsightsMessage.WARNING;
                data.message     = $t('Description contains %1 words, which is inadequate' +
                    ' (Recommended: equal or greater than 300 words).').replace('%1', wordNumber);
            }
            this.mp_queue.push(data);
        ...
        determineOutLink: function (description, baseUrl) {
            ...
            if (!hasOutLink) {
                ...
                data.state_class = SeoInsightsMessage.WARNING;
                data.message     = $t('Description has %1 outbound links' +
                    ' (Recommended: from 1 to 3 outbound links).').replace('%1', number);
            }
            this.mp_queue.push(data);
        ...
        inImageAlt: function (key, description, images) {
            ...
            if (failAttemp) {
                data.state_class = SeoInsightsMessage.WARNING;
                data.message     = $t('%1 product image(s) has(have) NO focus keyword in the description.' +
                    ' You should implement it..').replace('%1', failAttemp);
            } else {
                data.state_class = SeoInsightsMessage.SUCCESS;
            ...
            if (descriptionImages.length) {
                ...
                if (failAttemp) {
                    this.mp_queue.push({
                        ...
                        message: $t('%1 image(s) don\'t contain the main keyword in the product description.' +
                            ' You should implement it.').replace('%1', failAttemp)
                    });
                }
            }
