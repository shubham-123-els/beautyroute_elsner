...
class Data extends AbstractHelper
{
        ...
        foreach ($match[1] as $key => $attribute) {
            if ($type == Type::PAGES) {
                ...
                if ($attribute == 'category_name' && is_array($product->getCategoryIds())) {
                    ...
                    $productData[$match[0][$key]] =
                        $this->categoryFactory->create()->load($product->getCategoryIds()[$randomCategory])->getName();
    ...
    {
        $posts = ObjectManager::getInstance()->create(\Mageplaza\Blog\Model\Post::class)
            ->getCollection()->setPageSize($limit);
        $posts->getSelect()->orderRand();
