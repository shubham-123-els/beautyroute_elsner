...
class Notification extends CheckoutCart
{
    ...
    public function removeInvalidItem()
    {
        ...
        foreach ($quoteItems as $quoteItem) {
            ...
            if (!$itemRuleId && $isNoFgProductSpecialPrice
                && $quoteItem->getAppliedRuleIds() && $quoteItem->getDiscountAmount() > 0) {
                if ($quoteItem->getProductType() !== TypeConfigurable::TYPE_CODE
                    || ($isNoFgConfigurableProductSpecialPrice
                        && $quoteItem->getProductType() === TypeConfigurable::TYPE_CODE)) {
                    $removeFreeGifts = true;
                }
            }
        }
        ...
        foreach ($quoteItems as $quoteItem) {
            ...
            if ($itemRuleId && $isNoFgProductSpecialPrice
                && $quoteItem->getAppliedRuleIds() && ($quoteItem->getDiscountAmount() > 0 || $removeFreeGifts)) {
                if ($quoteItem->getProductType() !== TypeConfigurable::TYPE_CODE
                    || ($isNoFgConfigurableProductSpecialPrice
                        && $quoteItem->getProductType() === TypeConfigurable::TYPE_CODE)) {
