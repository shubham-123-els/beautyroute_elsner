...
class Gift
{
    ...
    public function isGiftAdded($giftId, $quoteId = null)
    {
        ...
        $quoteItem->addFieldToFilter(HelperRule::QUOTE_RULE_ID, ['notnull' => true]);

        if ($product->getTypeId() === Grouped::TYPE_CODE) {
            $ids = $this->groupProduct->getChildrenIds($giftId);
            $quoteItem->addFieldToFilter('product_id', ['in' => $ids])
                ->addFieldToFilter('parent_item_id', ['null' => true]);
        } else {
            $quoteItem->addFieldToFilter('product_id', $giftId);
            if ($product->getTypeId() === Type::TYPE_SIMPLE) {
                $quoteItem->addFieldToFilter('parent_item_id', ['null' => true]);
            }
        }

        if ((bool) $quoteItem->getSize()) {
            if ($product->getTypeId() === Grouped::TYPE_CODE) {
                return $quoteItem->getSize();
            }
     ...
     * @param string $ruleId
     * @param null $quoteId
     * @param int $countGroupProducts
     *
     * @return bool
     * @throws NoSuchEntityException
     */
    public function isMaxGift($ruleId, $quoteId = null, $countGroupProducts = 0)
    {
        $quoteItem = $this->getCurrentQuoteItems($quoteId);
        ...
        if ($this->helperData->getConfigGeneral('multi_free_gifts')) {
            $totalFreeGifts = 0;
            foreach ($quoteItem->getItems() as $item) {
                $totalFreeGifts += $item->getQty();
            }

            return $totalFreeGifts + $countGroupProducts > $rule->getMaxGift();
        }

        return $countGroupProducts ? ($quoteItem->getSize() + $countGroupProducts) > $rule->getMaxGift()
            : $quoteItem->getSize() >= $rule->getMaxGift();
    }
     ...
     * @throws NoSuchEntityException
     */
    public function getQtyGiftAdded($giftId, $quoteId = null)
    {
        $qty       = 0;
        $product   = $this->getProductById($giftId);
        $quoteItem = $this->getCurrentQuoteItems($quoteId);
