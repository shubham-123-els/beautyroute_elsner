...
class Rule extends AbstractModel
{
    ...
    public function getMaxGift()
    {
        ...
        $totalGifts  = $this->getCountGifts($this->getGiftArray());
    ...
    public function getCountGifts($gifts)
    {
        ...
        foreach ($gifts as $key => $gift) {
            $productGift = $this->helperGift->getProductById($key);
            if ($productGift->getTypeId() === Grouped::TYPE_CODE) {
                $ids   = $this->helperGift->getGroupProduct()
                    ->getChildrenIds($key)[Link::LINK_TYPE_GROUPED];
                $count += (count($ids) - 1);
            }
