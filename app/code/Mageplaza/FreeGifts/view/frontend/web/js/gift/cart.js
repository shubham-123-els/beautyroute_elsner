...
define([
    ...
    return Component.extend({
                ...
                quote.totals.subscribe(function (totals) {
                    ...
                    if (cartRules !== undefined) {
                        ...
                        _.each(cartRules, function (cartRule) {
                            self.rules.push(JSON.parse(JSON.stringify(cartRule)));
                            self.rule_list.push(JSON.parse(JSON.stringify(cartRule)));
                        });
                    }
        ...
        initAddOptionBtn: function (giftId, ruleId) {
            helper.createQty(this, giftId, ruleId);
        ...
        changeQty: function (self, ruleId, gift) {
            var element  = '#mpfreegifts-modal-item-' + ruleId,
                inputQty = $(element).find('.mpfreegifts-gift-input-qty-' + gift.sku);

            if (!parseInt(inputQty.val()) || inputQty.val() <= 0) {
                inputQty.val(1);
            }

            if (!gift.added) {
                if (inputQty.val() > self.calculateItemLeft()) {
                    inputQty.val(self.calculateItemLeft());
                }
            } else {
                if (!gift.qty) {
                    gift.qty = gift.qty_added;
                }
                if (gift.qty_added && inputQty.val() > gift.qty) {
                    inputQty.val(gift.qty_added);
                }
            }
        },
        ...
        giftAction: function (self, ruleId, url, itemId, gift) {
            var element         = '#mpfreegifts-modal-item-' + itemId,
                ...
                qty             = $(element).find('.mpfreegifts-gift-input-qty-' + gift.sku).val(),
                isRemoveGrouped = false;

            if ($(optionModal).length === 1
                && $(optionModal).data().mageModal
                && !$(element).closest('.mpfreegifts_modal').hasClass('_show')
            ) {
                helper.toggleGiftModal(optionModal, helper.option.title);
            } else {
                if (gift.is_grouped && url.indexOf('mpfreegifts/gift/remove') > -1) {
                    isRemoveGrouped = true;
                    url             = gift.option_url;
                }
                helper.giftAction(self, url, optionModal, {rule_id: ruleId, gift_id: gift.id, qty: qty, is_remove_grouped: isRemoveGrouped, is_cart: true}, element);
            }
        },
